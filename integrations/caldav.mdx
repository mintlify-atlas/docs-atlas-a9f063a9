---
title: "Connect CalDAV Calendars to Tymeslot"
sidebarTitle: "CalDAV"
description: "Sync Tymeslot with any CalDAV server: Nextcloud, Radicale, Zimbra, Baikal, Apple iCloud, and mailbox.org. No OAuth app required — uses username and password."
---
...
| **Default sync interval** | 15-minute fallback polling | 15-minute fallback polling | 15 minutes |

CalDAV does not support server-initiated push webhooks. Tymeslot polls CalDAV calendars on a 15-minute cycle regardless of whether `WEBHOOK_BASE_URL` is set. This is fully functional — availability and conflict checks remain accurate within the polling window.
...
CalDAV calendars are polled every **15 minutes**. This interval is fixed — CalDAV servers do not support push notifications that would allow Tymeslot to receive change events in real time. All conflict checks at the moment of booking use the most recent cached state, so in practice double-bookings are prevented even between polling cycles.
